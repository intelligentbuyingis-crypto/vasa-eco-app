...
const emptyJournal = (p?: ProjectDetails, samplerName = ""): FieldJournalData => ({
  ...
  pid: "", pidOpenAir: "",
  weather: [],
  labCalibValid: "", dailyCalib: "", coldStorage: "",
  samples: [], signature: "",
...
export default function Dashboard({ user, onLogout }: Props) {
  ...
  const handleProjectSelectedForField = (project: ProjectDetails) => {
    setActiveProject(project);
    setJournalData(emptyJournal(project, user.name));
    setScreen("field");
  };

  const handleProjectSelectedForChain = (project: ProjectDetails) => {
    setActiveProject(project);
    setJournalData(emptyJournal(project, user.name));
    setScreen("chain");
  };
  ...
  if (screen === "project-select-field") return (
    <ProjectSelector userName={user.name} onSelect={handleProjectSelectedForField} onBack={() => setScreen("dashboard")} />
  );
  if (screen === "project-select-chain") return (
    <ProjectSelector userName={user.name} onSelect={handleProjectSelectedForChain} onBack={() => setScreen("dashboard")} />
  );
  if (screen === "field") return (
    <FieldJournalForm user={user} data={journalData} onChange={setJournalData}
      onBack={() => setScreen("dashboard")} onContinue={() => setScreen("chain")} />
  );
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-green-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onLogout} className="text-xs text-green-300 border border-green-700 rounded px-2 py-1 hover:bg-green-800">יציאה</button>
        </div>
      </header>
      <main className="p-4 max-w-2xl mx-auto">
        <h2 className="text-gray-700 font-medium mb-4">בחר פעולה</h2>
        <div className="grid grid-cols-1 gap-3">
          <button onClick={() => setScreen("project-select-field")}
          ...
          <button onClick={() => setScreen("project-select-chain")}
            className="card text-right flex items-start gap-4 hover:border-green-200 hover:shadow-md transition-all">
          ...
          {user.isAdmin && (
            <button onClick={() => setScreen("admin")}
              ...
              <div>
                ...
                <div className="text-sm text-gray-500 mt-0.5">ניהול משתמשים · Admin בלבד</div>
              </div>
            </button>
          )}
        </div>
